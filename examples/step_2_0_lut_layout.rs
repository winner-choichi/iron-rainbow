...
use iron_rainbow::{Renderer2D, lut::{LutConfig, FalseColorStop}};
use image::Rgb;
use std::fs;
...
fn interpolate_false_color(stops: &[FalseColorStop], wavelength: f32) -> [u8; 3] {
    ...
    for pair in stops.windows(2) {
        ...
        if wavelength >= a.wavelength && wavelength <= b.wavelength {
            ...
            return [mix(a.color[0], b.color[0]), mix(a.color[1], b.color[1]), mix(a.color[2], b.color[2])];
        }
    }
...
fn main() {
    ...
    let config = LutConfig::load(CONFIG_PATH)
        .expect("Failed to load configs/lut_config.toml");

    println!("Step 2.0: LUT Layout Preview");
    println!("============================\n");
    println!("Loaded configuration from {}", CONFIG_PATH);
    println!("Wavelength axis: {:.1}–{:.1} nm ({} rows)",
             config.grid.wavelength_min_nm,
             config.grid.wavelength_max_nm,
             config.grid.wavelength_steps);
    println!("Angle axis: {:.1}°–{:.1}° ({} columns)",
             config.grid.angle_min_deg,
             config.grid.angle_max_deg,
             config.grid.angle_steps);
    println!("Angle supersample: {}x",
             config.grid.angle_supersample.unwrap_or(1));
    println!("Impact parameters: {:.2}–{:.2} ({} samples)",
             config.droplet.impact_min,
             config.droplet.impact_max,
             config.droplet.impact_samples);
    println!("Wavelength step: {:.2} nm (batch size {}), rays/λ: {}",
             config.wavelength.wavelength_step_nm,
             config.wavelength.batch_size,
             config.droplet.rays_per_wavelength);
    println!("Output format: {} | texture: {} | metadata: {}",
             config.output.format,
             config.output.texture_path,
    ...
    renderer.fill_rect(lut_x0, lut_y1, lut_x1 - lut_x0, lut_y1 - lut_y0, Rgb([230, 236, 247]));
        ...
        .enumerate()
    {
        ...
        renderer.draw_text_screen(tick_px as f32 - 18.0, base_y + offset, &text, 22.0, Rgb([60, 60, 90]));
    }
        ...
        .enumerate()
    {
        ...
        renderer.draw_text_screen(base_x + 20.0, tick_py as f32 + offset, &text, 22.0, Rgb([60, 60, 90]));
    }

    // Axis labels with dedicated margins (screen space)
    renderer.draw_text_screen(lut_px0 as f32 + 100.0, lut_py1 as f32 - 80.0, "Wavelength axis (145–200 nm)", 30.0, Rgb([30, 40, 90]));
    renderer.draw_text_screen(lut_px0 as f32 + 80.0, lut_py0 as f32 + 120.0, "Exit angle axis (-180° to -120°)", 30.0, Rgb([30, 40, 90]));
    renderer.draw_arrow(lut_x0, lut_y0 - 0.8, lut_x1, lut_y0 - 0.8, Rgb([60, 70, 110]));
    renderer.draw_arrow(lut_x0 - 0.8, lut_y0 - 0.2, lut_x0 - 0.8, lut_y1, Rgb([60, 70, 110]));
    renderer.draw_text_screen(lut_px1 as f32 - 220.0, lut_py0 as f32 - 60.0, "Observation angle", 24.0, Rgb([60, 70, 110]));
    renderer.draw_text_screen(lut_px0 as f32 - 220.0, lut_py1 as f32 + 30.0, "Wavelength", 24.0, Rgb([60, 70, 110]));
    ...
    for (idx, wl) in callout_wls.iter().enumerate() {
        ...
        renderer.draw_text_screen(tick_px as f32 - 220.0, tick_py as f32 + 10.0 + idx as f32 * 24.0,
            &format!("{} nm", wl), 22.0, Rgb([60, 60, 90]));
    }
    ...
    for (idx, angle) in callout_angles.iter().enumerate() {
        ...
        renderer.draw_text_screen(tick_px as f32 - 18.0, tick_py as f32 + 70.0 + idx as f32 * 24.0,
            &format!("{}°", angle), 22.0, Rgb([60, 60, 90]));
    }
    ...
    renderer.draw_text_screen(legend_px as f32 - 40.0, legend_py as f32 - 70.0, "False-color mapping", 26.0, Rgb([30, 30, 30]));
    ...
    if !config.renderer.false_color.is_empty() {
        ...
        for (i, stop) in config.renderer.false_color.iter().enumerate() {
            ...
            let (label_px, label_py) = renderer.world_to_pixel_coords(legend_x + 1.6, (y0 + y1) / 2.0);
            renderer.draw_text_screen(label_px as f32 + 4.0, label_py as f32 + 6.0,
                &format!("{} nm", stop.wavelength), 22.0, Rgb([40, 40, 40]));
        }
        renderer.draw_thick_line(legend_x, legend_y_top - legend_height, legend_x, legend_y_top, 0.03, Rgb([20, 20, 20]));
        renderer.draw_thick_line(legend_x + 1.5, legend_y_top - legend_height, legend_x + 1.5, legend_y_top, 0.03, Rgb([20, 20, 20]));
    }

    renderer.draw_text_screen(120.0, height as f32 - 100.0,
        &format!("Normalize: {} / Exposure: {:.1}",
            config.intensity.normalize_mode, config.intensity.exposure),
        26.0, Rgb([80, 80, 80]));
    renderer.draw_text_screen(120.0, height as f32 - 60.0,
        &format!("Droplet radius: {:.0} nm | Rays/λ: {}",
    ...
    renderer.save(output_path).expect("Failed to save LUT layout preview");
    println!("\n✓ Saved: {}", output_path);
}
