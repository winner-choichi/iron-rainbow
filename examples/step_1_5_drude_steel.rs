...
fn main() {
    ...
    renderer.draw_text(-5.8, 0.0, "n k", 0.22, axis_color);
    ...
    while y_val <= n_k_max + 0.1 {
        ...
        if y >= y_min && y <= y_max {
            ...
            renderer.draw_text(x_min - 0.5, y - 0.08, &label, 0.16, axis_color);
        }
        y_val += y_step;
    }

    // Legend (smaller, better positioned)
    renderer.draw_thick_line(3.2, 4.8, 3.6, 4.8, 0.04, n_color);
    renderer.draw_text(3.7, 4.8, "n", 0.2, n_color);
    renderer.draw_thick_line(3.2, 4.4, 3.6, 4.4, 0.04, k_color);
    renderer.draw_text(3.7, 4.4, "k", 0.2, k_color);
