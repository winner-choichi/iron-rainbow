...
impl Renderer2D {
    ...
    pub fn draw_arc(&mut self, cx: f32, cy: f32, radius: f32, start_angle: f32, end_angle: f32, color: Rgb<u8>) {
        let num_segments = 50;
    ...
    pub fn draw_thick_line(&mut self, x0: f32, y0: f32, x1: f32, y1: f32, thickness: f32, color: Rgb<u8>) {
        let dx = x1 - x0;
        let dy = y1 - y0;
        let len = (dx * dx + dy * dy).sqrt();
    ...
    /// Draw a dashed line
    pub fn draw_dashed_line(&mut self, x0: f32, y0: f32, x1: f32, y1: f32, dash_length: f32, color: Rgb<u8>) {
        let dx = x1 - x0;
        let dy = y1 - y0;
        let len = (dx * dx + dy * dy).sqrt();
